...
const { FILENAME_SUFFIX_VDOM, FILENAME_SUFFIX_INSTANCES } = require("../constants/paths")
...
const writeInstances = (eofolInstances, internalDir, viewName) => {
  ...
  const targetInstancesPath = path.resolve(internalDir, targetInstancesFilename)
  fs.writeFileSync(targetInstancesPath, JSON.stringify(eofolInstances, null, config.minifyRegistryJSON ? 0 : 2))
}
...
const writeVdom = (vdom, internalDir, viewName) => {
  ...
  fs.writeFileSync(targetVDOMPath, JSON.stringify(vdom[0], null, config.minifyRegistryJSON ? 0 : 2))
}
